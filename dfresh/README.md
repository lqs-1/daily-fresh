...
```html
在配置文件中：
    有一个自定义文存储的配置`DEFAULT_FILE_STORAGE`, 这个配置有两个选项，如果想自己定义的话可以选择其中一个，第一个选择是fdfs，
...
在配置全文检索的时候:
    ...
    在 haystack.backends.whoosh_backend 的同级目录中添加一个 whoosh_cn_backend的py文件,这个文件用于替换whoosh_backend,
    但是别把whoosh_backend删除，同时在whoosh_cn_backend中添加如下代码

更换分词器（jieba）：
    在/site-packages/haystack/backends下创建ChineseAnalyzer.py，内容如下代码
    在/site-packages/haystack/backends下复制一份whoosh_backend.py并且改名为whoosh_cn_backend.py,然后修改里面analyzer=
    ChineseTokenizer(),先引入，再使用在配置文件中修改使用的whoosh引擎为whoosh_cn_backend
